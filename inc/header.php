...
if(($CURRENT_PAGE != "login") && (Mage::getModel('core/cookie')->get("pg_user") == "")){
	header("Location: http://www.vapornation.com/prodguide/login");
}
...
if(($CURRENT_PAGE == "login") && (Mage::getModel('core/cookie')->get("pg_user"))){
	header("Location: http://www.vapornation.com/prodguide");
}

$submitted_password = $_POST["password"];

if(isset($submitted_password) && ($_POST['function']=="pg_login")) {
	
	$array_of_passwords = array(
		"bunny_rabbits_84",				// Master Password
		"vn_wholesale_trainer_34",		// Wholesale
		"vn_retail_trainer_65",			// Retail
		"test" 						  		// Easy Password For Testing
		);
	
	$valid = false;
	$error_message = '<div style="color:red">You entered invalid login credentials. Please try again.</div>';
	...
	//$valid = in_array($submitted_password, $array_of_passwords);
	
	if($valid){
		$name = "pg_user";
		$value = $submitted_password;
		$pass = Mage::getModel('core/encryption')->encrypt($value);
		Mage::getModel('core/cookie')->set($name, $pass);	
		//die("HERE");
		header("Location: http://www.vapornation.com/prodguide");			
	} 
	else {
		//header("Location: http://www.vapornation.com/prodguide/login");
		header("Location: http://www.vapornation.com/prodguide/login?attempt=failed");
	}
}
...
<body<?=$additional_body;?> data-spy="scroll" style="height:100%" data-target=".scrollspy">
<?php
